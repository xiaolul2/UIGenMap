...
class FormatBundleMap(object):
    ...
    def __call__(self, results):
        ...
        if 'pv_vectors' in results:
            # vectors may have different sizes
            vectors = results['pv_vectors']
            results['pv_vectors'] = DC(vectors, stack=False, cpu_only=True)
        ...
        if 'gt_depth' in results:
            ...
            gt_depth = np.ascontiguousarray(np.stack(gt_depth, axis=0))
            results['gt_depth'] = DC(to_tensor(gt_depth), stack=True)
