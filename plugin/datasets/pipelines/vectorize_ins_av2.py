...
class VectorizeMap_ins_av2(object):
    ...
    def draw_visible_polyline_cv2(self,cam_index,line, valid_pts_bool, image, color, thickness_px):
        ...
        for i in range(len(line) - 1):
            ...
            # print(image)
            # cv2.imwrite(f'av2_pv_GT_right/batch_{self.count}_{cam_index}_projection.png', image)
            # if np.any(image != 0):
            #     print("画上了")
    ...
    def get_semantic_mask(self, input_dict):
        ...
        gt_pv_semantic_mask = np.ones((num_cam, 1,608, 608), dtype=np.uint8)*255
        ...
        cam_extrinsics = input_dict['cam_extrinsics']
        cam_intrinsics = input_dict['cam_intrinsics']
        cam_intrinsics[1:] = [rot_resize_matrix[:3, :3] @ l2i for l2i in cam_intrinsics[1:]]
        cam_intrinsics[0] = rot_resize_matrix_1[:3, :3] @ cam_intrinsics[0]
        # category configs
        cat2id = {
            'ped_crossing': 0,
            'divider': 1,
           ...
           # ins_cls = cat2id[label]
            ...
            for vector in vector_list:
                ...
                for cam_index in range(num_cam):
                    self.line_ego_to_pvmask(cam_index,vector, gt_pv_semantic_mask[cam_index][0], cam_extrinsics[cam_index], cam_intrinsics[cam_index],color=1, thickness=1)
                    # 保存图像到文件
    ...
    def get_pv_vectorized_lines(self, input_dict):
        num_cam=len(input_dict['img_filenames'])
        gt_pv_semantic_mask = np.zeros((num_cam, 1,608, 608), dtype=np.uint8)
        gt_pv_ins_mask = np.zeros((num_cam, 100,38, 38), dtype=np.uint8)
        # scale_factor = np.eye(4)
        # scale_factor[0, 0] *= 1/32
        # scale_factor[1, 1] *= 1/32
        # ego2feat = [scale_factor @ l2i for l2i in input_dict['ego2img']]
        ...
        # cam_intrinsics[0] = rot_resize_matrix_1[:3, :3] @ cam_intrinsics[0]
        cam_intrinsics_new = []
        cam_intrinsics_front = rot_resize_matrix_1[:3, :3] @ cam_intrinsics[0]
        cam_intrinsics_new.append(cam_intrinsics_front)
        for i in range(1,len(cam_intrinsics)):
            cam_intrinsics_new.append(rot_resize_matrix[:3, :3] @ cam_intrinsics[i])
      #  np.set_printoptions(suppress=True)
       # print("input_dict['cam_extrinsics']",input_dict['cam_extrinsics'][0])

        # print("visav2_ex",cam_extrinsics)
     #   print("input_dict['cam_intrinsics']",input_dict['cam_intrinsics'][0])
        # print("visav2_ins",cam_intrinsics_new[0])
        # category configs
        cat2id = {
            'ped_crossing': 0,
            'divider': 1,
        ...
        for cam_index in range(num_cam):
            ...
            for label, vector_list in input_dict['pure_lines'].items():
                ...
                for vector in vector_list:
                    if isinstance(vector, list):
                        vector = np.array(vector)
                    pv_vector = self.ego2pv_line(vector, cam_extrinsics[cam_index], cam_intrinsics_new[cam_index])
                    if isinstance(pv_vector, np.ndarray) and pv_vector.shape[0]>=2:
                        #pv_vector = interp_utils.interp_arc(t=20, points=pv_vector) #608 608
                        #pv_vector = self.remove_nan_values(pv_vector)
                        pv_linestring= LineString(pv_vector)
                        
                        pv_vector = self.sample_fn(pv_linestring)
                        #print(pv_vector)
                        normed_pv_vector = pv_vector/(608, 608) ### 直接归一化
