...
class MapTransformerDecoder_new(BaseModule):
    ...
    def __init__(self, 
        ...
        #self.dynamic_positional_encoding = DynamicPositionalEncoding(256, max_len=20)
        #self.beta_embedding = nn.Sequential(
        #    nn.Linear(40, 256),
        #    nn.ReLU(),
        #    nn.Linear(256, 256),
        #)
    ...
    def forward(self,
        ...
        for lid, layer in enumerate(self.layers):
            ...
            #query_pos = self.dynamic_positional_encoding(tmp)
            #if reg_betas!=None:
            #    beta_embedding = self.beta_embedding(reg_betas.view(bs, num_queries, -1))
            #    query_pos = torch.cat((query_pos, beta_embedding),dim=-1).permute(1, 0, 2)
            #else:   
            #query_pos = self.linear(query_pos).permute(1, 0, 2)
            #    #query_pos = query_pos
            ...
            reg_points = reg_output[:, :, 0:40] # (bs, num_q, 2*num_points)
            reg_betas = reg_output[:, :, 40:80]

            #beta_embedding = self.beta_embedding(reg_betas).permute(1, 0, 2)
            #query_pos = self.dynamic_positional_encoding(tmp)
            #query_pos = torch.cat((query_pos, beta_embedding),dim=-1)
            #query_pos = query_pos + beta_embedding
            #print(reg_betas,"test betas")
            # breakpoint()
            bs, num_queries, num_points2 = reg_points.shape
            reg_points = reg_points.view(bs, num_queries, num_points2//2, 2) # range (0, 1)
            reg_betas = reg_betas.view(bs, num_queries, num_points2//2, 2) # range (0, 1)
            # breakpoint()
            ...
            # reg_betas = reg_betas.clone().detach()
            ...
            if self.return_intermediate:
                ...
                # breakpoint()
                intermediate_reference_points.append(torch.cat([new_reference_points, reg_betas], dim=-1)) # (bs, num_q, num_points, 2)
...
@TRANSFORMER_LAYER_SEQUENCE.register_module()
class MapTransformerDecoder_PV_un(BaseModule):
    """Implements the decoder in DETR transformer.
    Args:
        return_intermediate (bool): Whether to return intermediate outputs.
        coder_norm_cfg (dict): Config of last normalization layer. Default:
