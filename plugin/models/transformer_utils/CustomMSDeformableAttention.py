...
class MLP(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim, output_dim, num_layers, need_p=False):
        super().__init__()
...
class CustomMSDeformableAttention(BaseModule):
    ...
    def __init__(self,
        ...
        #self.mlp_mean = MLP(embed_dims, embed_dims, num_heads * num_levels * num_points, 2)
        #self.mlp_std = MLP(embed_dims, embed_dims, num_heads * num_levels * num_points, 2, True)
        self.mlp_attention = MLP(embed_dims, embed_dims, num_heads * num_levels * num_points*2, 2)
        self.softplus = nn.Softplus()
        self.value_proj = nn.Linear(embed_dims, embed_dims)
        self.output_proj = nn.Linear(embed_dims, embed_dims)
        self.init_weights()
...
#
        ...
        attention_params = self.mlp_attention(query).view(bs, num_query, self.num_heads*self.num_levels * self.num_points, 2)
        mean = attention_params[...,0]#self.mlp_mean(query)
        std = self.softplus(attention_params[...,1])#self.mlp_std(query)

        gaussian = torch.distributions.Normal(mean, std)
        alpha = gaussian.rsample()

        attention_weights = F.softmax(alpha, dim=-1).view(bs, num_query, self.num_heads, self.num_levels, self.num_points)
        ...
        #print(reference_points.shape, "test in attention")
