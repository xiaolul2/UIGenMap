...
class MapDetectorHead_2D(nn.Module):
    ...
    def forward_train(self, bev_features, img_metas, gts, pv_gts):
        '''
        Args:
...
#
        ...
        for i in range(len(bev_features)):
            bev_feature = bev_features[i]
            bs, num_cam, C, H, W = bev_feature.shape
            bev_feature = self._prepare_context(bev_feature.view(bs*num_cam, C,  H,W))
            
            img_masks = bev_feature.new_zeros((bs*num_cam, H, W))
            bev_features_list.append(bev_feature)
            img_masks_list.append(img_masks.type(torch.bool))
            pos_embed_list.append(None)
            
        bs = bev_features_list[0].shape[0]
        ...
        inter_queries, init_reference, final_inter_references = self.transformer(
            mlvl_feats= bev_features_list,#[bev_features,],
            mlvl_masks= img_masks_list,#[img_masks.type(torch.bool)],
            query_embed=query_embedding,
            prop_query=prop_query_embedding,
            mlvl_pos_embeds=pos_embed_list, # not used
            memory_query=None,
        ...
        loss_dict, det_match_idxs, det_match_gt_idxs, gt_lines_list = self.loss(gts=pv_gts, preds=outputs)
        if self.streaming_query:
            query_list = []
            ref_pts_list = []
            gt_targets_list = []
...
#
        ...
        for i, (queries) in enumerate(inter_queries):
            ...
            scores = self.cls_branches[i](queries) # (bs, num_q, num_classes)
            #print(queries.shape,"test queries shape")
            #print(len(scores),"test len score")
            reg_points_list = []
            betas_list = []
            scores_list = []
            query_list = []
    ...
    def get_targets(self, preds, gts, gt_bboxes_ignore_list=None):
        ...
        # format the inputs
        gt_labels = gts['pv_labels']
        gt_lines = gts['pv_lines']
        gt_weights = gts['pv_valids']
