...
from mmcv.cnn.bricks.transformer import build_positional_encoding, MultiheadAttention
...
class new_reg_branch_laplace(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.fc(h)
        means = out[...,:40]
        stds = torch.clamp(F.softplus(out[...,40:80]), min=1e-5, max=10)
...
class MapDetectorHead(nn.Module):
    ...
    def __init__(self, 
        ...
        self.use_enhanced_bev = use_enhanced_bev
        if self.use_enhanced_bev:
        ...
        self.mlp = MLP(40, self.embed_dims*2, self.embed_dims, 3)
    ...
    def _init_embedding(self):
        ...
        self.reference_points_lane = nn.Embedding(self.num_queries, 40)

        self.query_embedding_lane = nn.Sequential(
            nn.Linear(40, self.embed_dims),
            nn.ReLU(),
            nn.Linear(self.embed_dims, self.embed_dims),
        )
        self.ptsbeta_query_embedding_lane = nn.Sequential(
            nn.Linear(20*512, self.embed_dims),
            nn.ReLU(),
            nn.Linear(self.embed_dims, self.embed_dims),
        )

        self.pts_encoder = nn.Sequential(
            nn.Linear(2, 128),
            nn.ReLU(),
            nn.Linear(128,256),
        )
        ...
        self.ptsbeta_encoder = nn.Sequential(
            nn.Linear(4, 256),
            nn.ReLU(),
            nn.Linear(256, 512),
        )
    ...
    def propagate(self, query_embedding, init_reference_points,img_metas, return_loss=True):
        ...
        for i in range(bs):
            ...
            if is_first_frame:
                ...
                if return_loss:
                    ...
                    # (num_prop, 2*num_pts)
                    trans_loss += self.loss_reg(torch.cat([pred, betas], dim=-1), normed_targets, weights, avg_factor=1.0)  
    ...
    def pv2bev_lines_ipm_thr(self, pv_lines, pv_betas, pos_index, img_metas):
        #bs, num_cam, _, _ = pv_lines.shape
        bs = len(img_metas)
        pv_vectors = pv_lines#.view(bs, num_cam, -1, 20, 2)

        bev_lines = []
        bev_betas = []
        #scale = torch.tensor([800, 480]).to(pv_lines.device)
        scale = torch.tensor(self.img_size).to(pv_lines.device)
        scale_ = torch.tensor([60, 30]).to(pv_lines.device)

        for i in range(bs):
            ins = img_metas[i]['cam_intrinsics']
            exs = img_metas[i]['cam_extrinsics']
            num_cam = len(ins)
            bs_lines = []
            bs_betas = []

            cam_ins_inverses = [torch.linalg.inv(torch.tensor(ins[n]).to(pv_lines.device)).float() for n in range(num_cam)]
            cam_exs_inverses = [torch.linalg.inv(torch.tensor(exs[n]).to(pv_lines.device)).float() for n in range(num_cam)]

            for n in range(num_cam):
                #cam_vectors = cur_pv_vectors[n]
                cam_vectors = pv_lines[pos_index[:,0]==i*num_cam+n]
                cam_betas = pv_betas[pos_index[:,0]==i*num_cam+n]
                # 找到不为0的点，并进行最近邻插值填充
                for vector_idx in range(cam_vectors.shape[0]):
                    vector = cam_vectors[vector_idx]
                    vector = vector * scale

                    coords_3d_ego = -self.ipm_transform(vector, cam_ins_inverses[n], cam_exs_inverses[n])  ##刚刚浅加了个负号
                    coords_3d_ego[..., 0] -= self.pc_range[0]
                    coords_3d_ego[..., 1] -= self.pc_range[1]
                    coords_bev = coords_3d_ego[..., :2] / scale_
           
                    bs_lines.append(coords_bev)
                    bs_betas.append(cam_betas[vector_idx])

            if bs_lines:  # 检查列表是否非空
                bev_lines.append(torch.stack(bs_lines))
            else:
                bev_lines.append(torch.tensor([]))
            if bs_betas:  # 检查列表是否非空
                bev_betas.append(torch.stack(bs_betas))
            else:
                bev_betas.append(torch.tensor([]))

        return bev_lines, bev_betas
    def ipm_transform(self,vector, ins, exs):
        # 图像坐标转换为齐次坐标
        homogenous_vector = torch.cat([vector, torch.ones(vector.shape[0], 1).to(vector.device)], dim=1)

        camera_coords = homogenous_vector @ ins.T 
        vehicle_coords = camera_coords @ exs[:3, :3].T 

        vehicle_coords[:, :2]= vehicle_coords[:, :2] / vehicle_coords[:, 2].unsqueeze(1)
        
        return vehicle_coords[:, :2]


    def pts_crossatt_enhanced_bev(self, pts_embed, bev_feature):
        bs, feat_dim, H, W = bev_feature.shape
        line_num, num_points, point_dim = pts_embed.shape[1:]
        feat_flatten = bev_feature.flatten(2).permute(2, 0, 1)  # 变为 (H*W, bs, feat_dim)

        pts_embed = pts_embed.view(1, line_num * num_points, point_dim).permute(1, 0, 2)  # 变为 (line_num*20, bs, 256)
        pts_embed = self.cross_att(query=pts_embed, key=feat_flatten, value=feat_flatten)

        enhanced_bev = self.cross_att(query=feat_flatten, key=pts_embed, value=pts_embed)
        enhanced_bev = enhanced_bev.permute(1, 2, 0).reshape(bs, feat_dim, H, W)
        
        return enhanced_bev, pts_embed.permute(1, 0, 2).view(1, line_num, num_points, point_dim)

    def crossatt_enhanced_bev(self, pts_embed, bev_feature):

        bs, feat_dim, H, W = bev_feature.shape
        feat_flatten = bev_feature.flatten(2).permute(2, 0, 1) #h*w, bs, embed_dims

        pts_embed = pts_embed.permute(1,0,2)
        pts_embed = self.cross_att(query=pts_embed, key=feat_flatten, value=feat_flatten)
        enhanced_bev = self.cross_att(query=feat_flatten, key=pts_embed, value=pts_embed)
        return enhanced_bev.permute(1, 2, 0).reshape(bs, feat_dim, H, W), pts_embed.permute(1,0,2)

    def crossatt_enhanced_query(self, query_2d, query):

        bs, num_q,dim = query_2d.shape
        
        query_embed = self.cross_att(query=query.permute(1,0,2), key=query_2d.permute(1,0,2), value=query_2d.permute(1,0,2))
        return query_embed.permute(1, 0,2)


    def forward_train(self, bev_features, img_metas, pv_preds, gts):
        '''
        Args:
        ...
        pv_lines = torch.stack(pv_preds['lines'])#bs*num_cam, 50,40
        pv_betas = torch.stack(pv_preds['betas'])#torch.Size([18, 50, 40])

        pv_scores, _ = torch.stack(pv_preds['scores']).max(-1)#bs*num_cam, 50


        pv_scores = pv_scores.sigmoid()
        pos = pv_scores > threshold  
        #neg = pv_scores < threshold

        selected_lines = pv_lines[pos].view(-1, 20, 2)
        selected_betas = pv_betas[pos].view(-1, 20, 2)

        #notselected_betas = pv_betas[neg].view(-1, 20, 2)


        pos_index = torch.nonzero(pv_scores > threshold)
        
        pv_ref_pts, pv_betas = self.pv2bev_lines_ipm_thr(selected_lines, selected_betas, pos_index, img_metas)#.view(bs, num_cam*topk_num,-1)
        init_reference_points = self.reference_points_lane.weight.repeat(bs, 1, 1)
        query_embeddings = self.query_embedding_lane(init_reference_points)
        ...
        for i in range(bs):
            if pv_ref_pts[i].nelement() == 0 or pv_betas[i].nelement() == 0:
                bev_feature_list.append(bev_features[i])
                query_embedding_list.append(query_embeddings[i])
                continue
            

            #pts_features = self.pts_encoder(pv_ref_pts[i])
            #beta_features = self.beta_encoder(pv_betas[i])
            pv_ref_pts[i] = torch.cat((pv_ref_pts[i], pv_betas[i]), dim=-1)
            pts_beta_feature = self.ptsbeta_encoder(pv_ref_pts[i])
            #pts_beta_feature = torch.cat((pts_features, beta_features), dim=-1)
            #pts_beta_feature = pts_features

            weights = 1 / (torch.norm(pv_betas[i], p=2, dim=-1) + 1e-8)  # 防止除以零
            weights = torch.exp(weights / weights.sum(dim=1, keepdim=True))

            base = 1.0
            scale = 0.4
            weights = base + (weights - weights.mean(dim=1, keepdim=True)) * scale

            weighted_feats = pts_beta_feature * weights.unsqueeze(-1)

            pts_beta_feature_flatten = weighted_feats.view(weighted_feats.size(0), -1).unsqueeze(0)


            query_embedding2d = self.ptsbeta_query_embedding_lane(pts_beta_feature_flatten)
####
            if self.use_enhanced_bev:
                # pts_512 = self.pts2feat(weighted_features.unsqueeze(0))
                bev_feature, _  = self.pts_crossatt_enhanced_bev(weighted_feats.unsqueeze(0), bev_features[i].unsqueeze(0))
                query_embedding = self.crossatt_enhanced_query(query_embedding2d, query_embeddings[i].unsqueeze(0))
                bev_feature_list.append(bev_feature.squeeze(0))
                query_embedding_list.append(query_embedding.squeeze(0))
        bev_features = torch.stack(bev_feature_list)
        query_embeddings = torch.stack(query_embedding_list)

        if self.streaming_query:
            query_embedding, prop_query_embedding, init_reference_points, prop_ref_pts, memory_query, is_first_frame_list, trans_loss = \
                self.propagate(query_embeddings, init_reference_points, img_metas, return_loss=True)
    ...
    def forward_test(self, bev_features, img_metas, pv_preds):
        '''
        Args:
            bev_feature (List[Tensor]): shape [B, C, H, W]
                feature in bev view
        ...
        threshold = 0.4
        pv_lines = torch.stack(pv_preds['lines'])
        pv_betas = torch.stack(pv_preds['betas'])
        pv_scores, _ = torch.stack(pv_preds['scores']).max(-1)
        pv_scores = pv_scores.sigmoid()
        pos = pv_scores > threshold  
        selected_lines = pv_lines[pos].view(-1, 20, 2)
        selected_betas = pv_betas[pos].view(-1, 20, 2)
        ...
        line_numbers = [pv_ref_pts[i].shape[0] for i in range(len(pv_ref_pts))]
        init_reference_list = []
        bev_feature_list = []
        query_embedding_list = []
        ...
        for i in range(bs):
            ...
            pv_ref_pts[i] = torch.cat((pv_ref_pts[i], pv_betas[i]), dim=-1)
            pts_beta_feature = self.ptsbeta_encoder(pv_ref_pts[i])#(line_num, 20, 512)

            weights = 1 / (torch.norm(pv_betas[i], p=2, dim=-1) + 1e-6)  # 防止除以零
            weights = torch.exp(weights / weights.sum(dim=1, keepdim=True))
            base = 1.0
            scale = 0.4
            weights = base + (weights - weights.mean(dim=1, keepdim=True)) * scale
...
########
        ...
        if self.streaming_query:
            query_embedding, prop_query_embedding, init_reference_points, prop_ref_pts, memory_query, is_first_frame_list = \
                self.propagate(query_embeddings, init_reference_points, img_metas, return_loss=False)   
        else:
            init_reference_points = init_reference_points.view(-1, self.num_queries, self.num_points, 2) # (bs, num_q, num_pts, 2)
            prop_query_embedding = None
            prop_ref_pts = None
